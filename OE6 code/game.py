...
pygame.display.set_caption("OE6 version Game")#set the title
...
#2nd condition sets the boundaries where u can only walk
    if(mkeys[0] and not play.isJump and len(bullets) <=0):
        ...
        play.l = False
        play.r = True
    elif (keys[pygame.K_w]and play.y > 200 and not play.isJump):#370 is the initial val of sprite
        if(play.ssoundCount ==0):
            step.play(0)
            play.ssoundCount = 30
        else:
